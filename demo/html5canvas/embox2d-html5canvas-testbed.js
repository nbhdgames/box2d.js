...
function onKeyDown(canvas, evt) {
  ...
  if (currentTest && currentTest.onKeyDown) currentTest.onKeyDown(canvas, evt);
...
function onKeyUp(canvas, evt) {
  ...
  if (currentTest && currentTest.onKeyUp) currentTest.onKeyUp(canvas, evt);
}
...
function updateDebugDrawCheckboxesFromWorld() {
  ...
  document.getElementById("drawShapesCheck").checked =
    (flags & e_shapeBit) != 0;
  document.getElementById("drawJointsCheck").checked =
    (flags & e_jointBit) != 0;
  ...
  document.getElementById("drawTransformsCheck").checked =
    (flags & e_centerOfMassBit) != 0;
}

function updateWorldFromDebugDrawCheckboxes() {
  var flags = 0;
  if (document.getElementById("drawShapesCheck").checked) flags |= e_shapeBit;
  if (document.getElementById("drawJointsCheck").checked) flags |= e_jointBit;
  if (document.getElementById("drawAABBsCheck").checked) flags |= e_aabbBit;
...
function createWorld() {
  ...
  currentTest = new window[v]();
  currentTest.setup();
}

function resetScene() {
  if (currentTest && currentTest.cleanup) {
    currentTest.cleanup();
  }
  createWorld();
  draw();
...
function updateStats() {
  ...
  fbSpan.innerHTML =
    "Status: " +
    (run ? "running" : "paused") +
    "<br>Physics step time (average of last 60 steps): " +
    myRound(frameTime60, 2) +
    "ms" +
    //"<br>Mouse down: "+mouseDown +
    "<br>PTM: " +
    myRound(PTM, 2) +
    "<br>View center: " +
    myRound(currentViewCenterWorld.x, 3) +
    ", " +
    ...
    mousePosPixel.y +
    "<br>Mouse pos (world): " +
    myRound(mousePosWorld.x, 3) +
    ", " +
    myRound(mousePosWorld.y, 3);
}
